  ...
  return (
    <DashboardLayout>
      <div className="page-header">
        <h1 className="text-3xl font-bold" style={{fontFamily:'Poppins,sans-serif'}}>Modules</h1>
        <p>Student &gt; Modules</p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1">Check back soon — admin will upload learning materials here.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {modules.map((mod, idx) => (
              <div key={mod._id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
                {/* Cover image */}
                <div className="h-48 overflow-hidden">
                  <img
                    src={mod.coverImage || MODULE_IMAGES[idx % MODULE_IMAGES.length]}
                    alt={mod.title}
                    className="w-full h-full object-cover"
                    onError={e => { e.target.src = MODULE_IMAGES[idx % MODULE_IMAGES.length]; }}
                  />
                </div>

                <div className="p-5">
                  <h3 className="font-bold text-indigo-800 text-base mb-1">{mod.title}</h3>
                  {mod.description && (
                  ...
                  >
                    {expanded === mod._id ? '▲ Hide resources' : '▼ View resources'}
                    <span className="text-xs text-gray-400 ml-1">({mod.resources?.length || 0} files)</span>
                  </button>
                  ...
                  {expanded === mod._id && (
                    <div className="mt-3 space-y-2 border-t pt-3">
                      ...
                      {mod.resources?.map(r => (
                        ...
                        >
                          <span className="text-lg">{FILE_ICONS[r.fileType] || FILE_ICONS.other}</span>
                          <span className="text-gray-700 hover:text-indigo-700 font-medium truncate">{r.fileName}</span>
                          <span className="ml-auto text-xs text-gray-400 uppercase">{r.fileType}</span>
                        </a>
                      ))}
                    </div>
                  )}

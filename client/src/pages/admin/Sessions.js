  ...
  return (
    <DashboardLayout>
      <div className="page-header">
        <h1 className="text-3xl font-bold" style={{fontFamily:'Poppins,sans-serif'}}>All Sessions</h1>
        <p>Admin &gt; Sessions ({sessions.length} total)</p>
      </div>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-5">
        <div className="grid grid-cols-4 gap-4">
          {[['All', 'all', 'bg-gray-700'], ['Upcoming','upcoming','bg-blue-600'], ['Completed','completed','bg-green-600'], ['Cancelled','cancelled','bg-red-500']].map(([l,k,c]) => (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>{['Topic','Mentor','Mentee','Status','Date','Time','Link'].map(h=>(
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    <tr key={s._id} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="py-3 px-4 text-sm font-medium text-gray-700 max-w-48 truncate">{s.topic}</td>
                      <td className="py-3 px-4 text-sm text-gray-600">{s.mentor?.name}</td>
                      <td className="py-3 px-4 text-sm text-gray-600">{s.mentee?.name}</td>
                      ...
                      <td className="py-3 px-4 text-sm text-gray-500">{s.startTime}</td>
                      <td className="py-3 px-4">
                        {s.meetingLink
                          ? <a href={s.meetingLink} target="_blank" rel="noreferrer" className="text-xs text-indigo-600 hover:underline">Join</a>

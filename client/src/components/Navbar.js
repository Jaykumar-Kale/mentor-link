...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'bg-white shadow-lg border-b border-gray-100' : 'bg-white/95 backdrop-blur-sm shadow-sm'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-18" style={{height:'72px'}}>
        ...
        <Link to="/" className="flex items-center gap-3 flex-shrink-0" onClick={() => setOpen(false)}>
          <img
            ...
            onError={e => { e.target.src = 'https://ui-avatars.com/api/?name=M&background=3730a3&color=fff&size=48'; }}
          />
          <div className="hidden sm:block">
            <p className="font-bold text-sm leading-tight" style={{color:'#3730a3',fontFamily:'Plus Jakarta Sans,sans-serif'}}>
          ...
          {[
            ...
            <Link key={to} to={to} onClick={() => setOpen(false)}
              className={`block px-4 py-3 rounded-lg text-sm font-semibold transition ${
                isActive(to) ? 'text-indigo-700 bg-indigo-50' : 'text-gray-600 hover:bg-gray-50'
              }`}>
              {label}
            </Link>
          ))}
          <div className="pt-2 border-t border-gray-100 space-y-2">
            {user ? (
              <>
                ...
                <button onClick={handleLogout} className="block w-full text-center text-red-500 py-2 font-semibold text-sm">Logout</button>
              </>
            ) : (
              <>
